...
def get_all_questions():
    if len(all_questions) > 0:
        return jsonify({
            "message":"All Forum Questions",
            "Available questions":[     
                question.__dict__ for question in all_questions
            ]
        }),200
    return jsonify({"message":"No questions have been posted"}), 404
...
def post_answer(question_id):
    ...
    for question in range(len(all_questions)):
        if ((all_questions[question]["qstn_id"]) == int(_id)):
            ...
            return jsonify({
            "message":"Answer has been posted to your question",
            "Question answered":[     
                all_questions[question]["question"]
            ]}),201
    return jsonify({
        "message":"Question not posted yet",
    }),400
