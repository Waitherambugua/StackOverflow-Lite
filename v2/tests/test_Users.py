"""
Test for the user class with authentication
...
class UserTestCase(unittest.TestCase):
    """This class runs checks for user test case"""
    def setUp(self):
        """Define test variables and initialize app."""
        self.app = create_app(config_name="testing")
        self.client = self.app.test_client
        self.user = {'email': 'flaskuser@email.com', 'password': 'flask2018'}
    ...
    def test_user_existence(self):
        ...
        user_data = {'email': 'flaskuser.email@com', 'password': 'flask2018'}
    ...
    def test_password_length(self):
        ...
        user_data = {'email': 'flaskuser@email.com', 'password': 'flask'}
        resp = self.client().post('/auth/register/', data=user_data)
    ...
    def test_reset_password(self):
        ...
        self.registration()
        self.user['password'] = 'python2018'
        resp = self.client().post('/auth/reset_password/', data=self.user)
        self.assertEqual(resp.status_code, 200)
        self.assertIn('Password was changed successfully', str(resp.data))
    ...
    def test_delete_user(self):
        ...
        resp = self.client().delete(
            '/auth/delete/', headers=self.login(), data=self.user)
        self.assertEqual(resp.status_code, 200)
        self.assertIn('User has been deleted', str(resp.data))
...
# Make the tests easily executable
